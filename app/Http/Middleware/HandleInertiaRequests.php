        ...
        return [
            ...
            'auth' => [
                ...
                'can' => [
                    ...
                    'inquiriesView' => $request->user()?->can('inquiries.view') ?? false,
                    'inquiriesUpdate' => $request->user()?->can('inquiries.update') ?? false,
                    'inquiriesDelete' => $request->user()?->can('inquiries.delete') ?? false,
                    'inquiriesAssign' => $request->user()?->can('inquiries.assign') ?? false,
                    'inquiriesRespond' => $request->user()?->can('inquiries.respond') ?? false,
                    'inquiriesApprove' => $request->user()?->can('inquiries.approve') ?? false,
                    'inquiriesSend' => $request->user()?->can('inquiries.send') ?? false,
                    'dictionariesView' => $request->user()?->can('dictionaries.view') ?? false,
                    'dictionariesCreate' => $request->user()?->can('dictionaries.create') ?? false,
                    'dictionariesUpdate' => $request->user()?->can('dictionaries.update') ?? false,
                    'dictionariesDelete' => $request->user()?->can('dictionaries.delete') ?? false,
